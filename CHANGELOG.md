...
### Fixed
- `insertID()` no longer calls `PDO::lastInsertId()`, which is unsupported by
  `pdo_firebird` and threw `SQLSTATE[IM001]: driver does not support lastInsertId()`.

### Added
- `Builder::_insert()` now appends a `RETURNING "<pk>"` clause to every single-row
  `INSERT` statement, allowing the driver to capture the generated or provided primary
  key value without relying on PDO's `lastInsertId()`.
- `Connection::getPrimaryKeyColumn(string $table): string` — looks up the primary key
  column for a given table via Firebird system tables (`RDB$RELATION_CONSTRAINTS` /
  `RDB$INDEX_SEGMENTS`). Result is cached per connection lifetime.
- `Connection::$lastInsertedId` property stores the ID fetched from the `RETURNING`
  clause; `insertID()` reads this property.
- Tests: `testInsertCapturesIdViaReturning()`, `testGetPrimaryKeyColumn()`, and
  `testModelInsertWithAutoIncrementReturnsCorrectId()` cover the new behaviour.
